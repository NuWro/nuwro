...
double e_spp_event(params &p, event &e, nucleus &t, bool nc) {
  ...
  double ctl = p.el_costh_lab;
  // if(fabs( costhetalab(l0,l1) - cos(p.eel_theta_lab*Pi/180)) > dz)
  if (fabs(costhetalab(l0, l1) - ctl) > dz)
    e.weight *= 1e-20;
  else
    e.weight /= (min(1.0, ctl + dz) - max(-1.0, ctl - dz)) /
                2; // this is the allowed/all ratio in costhetalab
...
double e_spp_event2(params &p, event &e, nucleus &t, bool nc) {
  ...
  double m2 = l0.mass2();
  double ctl = p.el_costh_lab;
  double tl = std::acos(ctl); // In radians
  // vec k = turn_by_theta(l0, eel_theta_lab * Pi/180.0);
  vec k = turn_by_theta(l0, tl);
  k = k.dir();
  double E = p4.t;
  double E2 = E * E;
  vec P = vec(p4);
  ...
  double delta = E2 * E2 - 2 * E2 * m2 + 4 * kp2 * m2 + m2 * m2 - 2 * E2 * M2 +
                 M2 * M2 - 2 * E2 * p2 - 2 * m2 * p2 + 2 * M2 * p2 + p2 * p2;
  ...
  double x[2] = {A * (B - E * sqrt(delta)), A * (B + E * sqrt(delta))};
...
double e_spp_event3(params &p, event &e, nucleus &t, bool nc) {
  ...
  //  electron  elastic scattering
  //
  // std::cerr << "In function e_spp_event3()...\n";
  e.weight = 0; // event impossible before setting non zero cross section
  ...
  double ctl = p.el_costh_lab;
  double theta = std::acos(ctl);
  // std::cout << "theta(deg)=" << theta * 180 / Pi << "\n";
  vec k1 = turn_by_theta(l0.p(), theta); // out mom dir
  l1.set_momentum(k1);

  vect sum = l0 + N0;
  /*
  vect sum2 = l0.p4() + N0.p4();
  std::cout << "sum=(" << sum.x << ", " << sum.y << ", " << sum.z << ", "
            << sum.t << ")\n";
  std::cout << "sum2=(" << sum2.x << ", " << sum2.y << ", " << sum2.z << ", "
            << sum2.t << ")\n";
  */
  ...
  */
  ...
  e.weight = dsigma_dq0_dOmegal_dOmegaCMS_Oset(
      l0.E(), e.in[0].mass(), D4V<double>(l1.t, l1.x, l1.y, l1.z), costhetacms,
      phicms, D4V<double>(N0.t, N0.x, N0.y, N0.z), KF,
      rho_rel); // in cm2 per
