...
int ConvertCoordinate_2D_to_1D(int n, int m) {
  // Convert the 2D co-ordinates of a N x N upper triangular matrix to a 1D
  // matrix (which stores only non zero elements) formula = n*(2N -n + 1 )/2  +
  // m-n  where N is the dimension of the upper triangular matrix
  return ((int)(n * (240 - n + 1) / 2 + m - n));
}

double Bilinear_Interpolation(double x, double y, double x1, double x2,
                              double y1, double y2, double f11, double f12,
...
void mecevent_2020Valencia(params &p, event &e, nucleus &t, bool cc) {
  ...
  double mc_sampling[3] = {
      0, 0, 0}; // sampling direction for pp, np, and pn respectively
  int mc_strength[3] = {
      1, 1, 1}; // strength for sampling direction pp, np, and pn respectively
  ...
  if (width_q0 > 0) {
    weight_2p2h = Valencia2020_kin_and_weight_2p2h(
        e.in[0].E(), individual_weight, meclepton, mecnucleon_in,
        mecnucleon_out, flag_2p2h_pn, t, mec_central, mec_smearing, binding,
        ile_pb, mc_sampling, mc_strength);

    weight_3p3h = Valencia2020_kin_and_weight_3p3h(
        e.in[0].E(), individual_weight, meclepton_3p3h, mecnucleon_3p3h_in,
        mecnucleon_3p3h_out, t, binding, ile_pb);
  }
...
double Valencia2020_kin_and_weight_2p2h(
    double E, double *individual_dsdqdw, particle &meclep,
    particle *inc_nucleon_2p2h, particle *out_nucleon_2p2h, bool &flag_2p2h_pn,
    nucleus &t, double mec_central, double mec_smearing, double binding,
    int ile_PB, double *sampling, int *strength) {
  // it is assumed that neutrino direction is (0,0,1); but transition to other
  // direction in nuwro.cc!

  // here we extrapolate the cross section from 2020 Valencia's grid file stored
  // in src/MEC_data/ pp.h ... and so on....
  double result = 0;
  ...
  if (cos_min < 1) {
    ...
    if ((q2 > q0 * q0)) {
      ...
      for (int outgoing_pair = 0; outgoing_pair < 3; outgoing_pair++) {
        individual_dsdqdw[outgoing_pair] =
            Valencia2020_dsdEdc(E, outgoing_pair, q0, Eout_lepton, ct, t) *
            width_q0 * width_cos;
      ...
      if (result > 0) {
        ...
        vec qq(cos(phi) * lp * sqrt(1.0 - ct * ct),
               sin(phi) * lp * sqrt(1.0 - ct * ct), E - lp * ct);

        // lepton momentum
        vec kprim(-cos(phi) * lp * sqrt(1.0 - ct * ct),
                  -sin(phi) * lp * sqrt(1.0 - ct * ct), lp * ct);
        ...
        double dsdqdw_pp =
            (individual_dsdqdw[0] > 0) ? individual_dsdqdw[0] : 0;
        double dsdqdw_np =
            (individual_dsdqdw[1] > 0) ? individual_dsdqdw[1] : 0;
        double dsdqdw_pn =
            (individual_dsdqdw[2] > 0) ? individual_dsdqdw[2] : 0;

        double ratio_pp = dsdqdw_pp / result;
        double ratio_np = ((dsdqdw_np) > 0 || (dsdqdw_pn > 0))
                              ? dsdqdw_np / (dsdqdw_np + dsdqdw_pn)
                              : 0;

        Isospin_model_2p2h_2020Valencia(inc_nucleon_2p2h, out_nucleon_2p2h,
                                        ratio_pp, ratio_np, flag_2p2h_pn);
        Generate_nucleon_kinematics_2p2h(
            inc_nucleon_2p2h, out_nucleon_2p2h, flag_2p2h_pn, t, result, qqq,
            mec_central, mec_smearing, binding, ile_PB, sampling, strength);
...
double Valencia2020_kin_and_weight_3p3h(double E, double *individual_dsdqdw,
                                        particle &meclep,
                                        particle *inc_nucleon_3p3h,
                                        particle *out_nucleon_3p3h, nucleus &t,
  ...
  if (cos_min < 1) {
    ...
    if ((q2 > q0 * q0)) {
      ...
      // double differential cross section
      result = Valencia2020_dsdEdc(E, 3, q0, Eout_lepton, ct, t) * (E - ml) *
               width_cos; // GeV;
      ...
      if (result > 0) {
        ...
        Generate_nucleon_kinematics_3p3h(inc_nucleon_3p3h, out_nucleon_3p3h, t,
                                         result, qqq, ile_PB);
...
double Valencia2020_dsdEdc(double E, int outgoing_pair, double q0, double Ep,
                           double ct, nucleus &T) {
  double result = 0;
  ...
  if ((vq2 > q0 * q0) && (vq2 > 0) && (fabs(ct) <= 1)) {
    ...
    if ((q0 <= q) and (q >= 0.01 * GeV) and (q0 >= 0) and (q <= qmax_Nieves)) {
      ...
      int l = outgoing_pair;
      int n = (q0 > spacing) ? (int)((q0 - spacing) / spacing)
                             : -1; // q0 = 10MeV, q = 10MeV corresponds to 0th
                                   // row and 0th column of the 2D matrix
      ...
      // x - direction corresponds to q (in MeV); i.e traversing column in the
      // 2D matrix y - direction corresponds to q0 (in MeV); i.e traversing row
      // in the 2D matrix k is the index number in 1D array corresponding to
      // index [n][m];
      ...
      if (n < 0) {
        ...
        w00 = Bilinear_Interpolation(q, q0, x1, x2, y1, y2, 0, 0,
                                     T.GetValue_W00(l, k3),
                                     T.GetValue_W00(l, k4));
        w03 = Bilinear_Interpolation(q, q0, x1, x2, y1, y2, 0, 0,
                                     T.GetValue_W03(l, k3),
                                     T.GetValue_W03(l, k4));
        w11 = Bilinear_Interpolation(q, q0, x1, x2, y1, y2, 0, 0,
                                     T.GetValue_W11(l, k3),
                                     T.GetValue_W11(l, k4));
        w12 = Bilinear_Interpolation(q, q0, x1, x2, y1, y2, 0, 0,
                                     T.GetValue_W12(l, k3),
                                     T.GetValue_W12(l, k4));
        w33 = Bilinear_Interpolation(q, q0, x1, x2, y1, y2, 0, 0,
                                     T.GetValue_W33(l, k3),
                                     T.GetValue_W33(l, k4));
        ...
        // << "\n"; cout << "Base point 2: (" << y1 << "," << x2 << ")\t Value =
        // " << 0 << "\n"; cout << "Base point 3: (" << y2 << "," << x1 << ")\t
        // Value = " << T.GetValue_W00(l, k3) << "\n"; cout << "Base point 4: ("
        // << y2 << "," << x2 << ")\t Value = " << T.GetValue_W00(l, k4) <<
        // "\n";
        ...
        if (n == m) {
          ...
          w00 = Bilinear_Interpolation(
              q, q0, x1, x2, y1, y2, T.GetValue_W00(l, k1),
              T.GetValue_W00(l, k2), 0, T.GetValue_W00(l, k4));
          w03 = Bilinear_Interpolation(
              q, q0, x1, x2, y1, y2, T.GetValue_W03(l, k1),
              T.GetValue_W03(l, k2), 0, T.GetValue_W03(l, k4));
          w11 = Bilinear_Interpolation(
              q, q0, x1, x2, y1, y2, T.GetValue_W11(l, k1),
              T.GetValue_W11(l, k2), 0, T.GetValue_W11(l, k4));
          w12 = Bilinear_Interpolation(
              q, q0, x1, x2, y1, y2, T.GetValue_W12(l, k1),
              T.GetValue_W12(l, k2), 0, T.GetValue_W12(l, k4));
          w33 = Bilinear_Interpolation(
              q, q0, x1, x2, y1, y2, T.GetValue_W33(l, k1),
              T.GetValue_W33(l, k2), 0, T.GetValue_W33(l, k4));
        } else {
          k1 = ConvertCoordinate_2D_to_1D(n, m);
          k2 = ConvertCoordinate_2D_to_1D(n, m + 1);
          k3 = ConvertCoordinate_2D_to_1D(n + 1, m);
          ...
          w00 = Bilinear_Interpolation(
              q, q0, x1, x2, y1, y2, T.GetValue_W00(l, k1),
              T.GetValue_W00(l, k2), T.GetValue_W00(l, k3),
              T.GetValue_W00(l, k4));
          w03 = Bilinear_Interpolation(
              q, q0, x1, x2, y1, y2, T.GetValue_W03(l, k1),
              T.GetValue_W03(l, k2), T.GetValue_W03(l, k3),
              T.GetValue_W03(l, k4));
          w11 = Bilinear_Interpolation(
              q, q0, x1, x2, y1, y2, T.GetValue_W11(l, k1),
              T.GetValue_W11(l, k2), T.GetValue_W11(l, k3),
              T.GetValue_W11(l, k4));
          w12 = Bilinear_Interpolation(
              q, q0, x1, x2, y1, y2, T.GetValue_W12(l, k1),
              T.GetValue_W12(l, k2), T.GetValue_W12(l, k3),
              T.GetValue_W12(l, k4));
          w33 = Bilinear_Interpolation(
              q, q0, x1, x2, y1, y2, T.GetValue_W33(l, k1),
              T.GetValue_W33(l, k2), T.GetValue_W33(l, k3),
              T.GetValue_W33(l, k4));
        }
      }
    ...
    result +=
        ml2 / ((Ep + lp) * Ep) *
        (w1 * ct - 0.5 * w2 * ct + 0.5 * w3 * (Ep * (1.0 - ct) + lp - E * ct) +
         0.5 * w4 * (ml2 * ct + 2 * Ep * (Ep + lp) * sts) -
         0.5 * w5 * (Ep + lp));
...
void Isospin_model_2p2h_2020Valencia(particle *in_p, particle *out_p,
                                     double ratio_pp, double ratio_np,
                                     bool &flag_2p2h_pn) {
...
void Isospin_model_3p3h_2020Valencia(particle *in_p, particle *out_p,
                                     nucleus &T_nucleus) {
  ...
  double ppp =
      (NP >= 3) ? (NP * (NP - 1) * (NP - 2)) / 6 : 0; // inital pairs npp
  double nnn =
      (NN >= 3) ? (NN * (NN - 1) * (NN - 2)) / 6 : 0; // inital pairs pnn

  double npp = (NP >= 2 && NN >= 1) ? ((NP * (NP - 1)) / 2) * NN
                                    : 0; // initial pairs nnp
  double pnn = (NN >= 2 && NP >= 1) ? ((NN * (NN - 1)) / 2) * NP
                                    : 0; // initial pairs ppn

  double nnp = (NN >= 2 && NP >= 1) ? ((NN * (NN - 1)) / 2) * NP
                                    : 0; // initial pairs nnn
  double ppn = (NP >= 2 && NN >= 1) ? ((NP * (NP - 1)) / 2) * NN
                                    : 0; // initial pairs ppp
...
void Generate_nucleon_kinematics_2p2h(particle *inc_nucleon_mec,
                                      particle *out_nucleon_mec,
                                      bool &flag_2p2h_pn, nucleus &T,
                                      double &xsec_result, vect qqq,
                                      double mec_central, double mec_smearing,
                                      double binding, int ile_PB,
                                      double *sampling, int *strength) {
  vect suma(vec(0, 0, 0),
            0); // 4-momentum vector of the whole system ... initial nucleon
                // pairs + 4-momentum energy transfer
  ...
  do {
    ...
    particle probe1; // <- dummy nucleon inside the nucleus . Randomly sampled
                     // between [0, r_max] weighted by the nuclear density !!
    vec N1 = vec(0, 0, 0);
    vec N2 = vec(0, 0, 0);
    double lengthN1 = 0;
    double lengthN2 = 0;
    do {
      ...
      // Arange such that N1 contains higher momentum and N2 contains lower
      // momentum
      vec temp_vector = N1;
      ...
      double Ek1_in = sqrt(std::pow(N1.length(), 2) + Mass1 * Mass1) -
                      Mass1; // <-- K.E of nucleon assuming first nucleon has
                             // higher momentum
      double Ek2_in = sqrt(std::pow(N2.length(), 2) + Mass2 * Mass2) -
                      Mass2; // <-- K.E of nucleon assuming second nucleon has
                             // lower momentum
      // ==> We provide an extra K.E of about ~ 15 MeV to initial nucleon. 0.8
      // fraction of the extra kinetic energy goes to nucleon with higher
      // momentum <==
      double E_KE = 0; // changed by JS in December 2025 due to weird structure
                       // in KDAR simulation; originally was 14.3*MeV
      double fraction = 0.5;
      double frac_Ek1_in = fraction * E_KE;
      double frac_Ek2_in = (1 - fraction) * E_KE;

      N1 = N1 *
           sqrt(std::pow(Ek1_in + Mass1 + frac_Ek1_in, 2) - Mass1 * Mass1) /
           N1.length();
      N2 = N2 *
           sqrt(std::pow(Ek2_in + Mass2 + frac_Ek2_in, 2) - Mass2 * Mass2) /
           N2.length();
    ...
    inc_nucleon_mec[0].r =
        probe1.r; // <- Get a random position of incoming nucleon 1.
    inc_nucleon_mec[1].r =
        probe1.r; // <- set the position of initial nucleon 2 at nucleon 1

    inc_nucleon_mec[0].set_momentum(
        N1); // <- set the momentum for initial nucleon 1
    inc_nucleon_mec[1].set_momentum(
        N2); // <- set the momentum for initial nucleon 2
             //
    inc_nucleon_mec[0].set_fermi(T.Ef(
        inc_nucleon_mec[0])); // local fermi energy is used in energy balance
    // inc_nucleon_mec[1].set_fermi(T.Ef(inc_nucleon_mec[1])); // local fermi
    // energy for initial nucleon 2

    //-------------------------------------------------------------------------------------------------------------------------------------------------------------//

    suma = inc_nucleon_mec[0].p4() + inc_nucleon_mec[1].p4() +
           qqq; // <-- Total 4-momentum sum of the initial system
    double bilans = inc_nucleon_mec[0].Ek() + inc_nucleon_mec[1].Ek() -
                    2 * (inc_nucleon_mec[0].his_fermi + 0.5 * binding);
    ...
    if ((suma * suma > pow(Mass3 + Mass4, 2)) &&
        (suma.t > suma.length())) { //  <-- Hadronic condition is checked here
      ...
      suma.boost2(trans); // <- boost to CM frame . Note! boost(2) boost in the
                          // reverese direction
      double Ecm3 = 0.5 * (suma.t * suma.t + Mass3 * Mass3 - Mass4 * Mass4) /
                    suma.t; // Energy of outgoing nucleon 1
      double Ecm4 = 0.5 * (suma.t * suma.t + Mass4 * Mass4 - Mass3 * Mass3) /
                    suma.t; // Energy of outgoing nucleon 2

      if (!PB) { // <-- A piece of code if the mec_pauli_blocking is turned of
                 // in data/params.txt
        ...
        dir_cm = rand_direc2(
            trans, pp * sampling[0] + np * sampling[1] + pn * sampling[2],
            pp * strength[0] + np * strength[1] + pn * strength[2], 1);
        dir_cm2 = -dir_cm; // <-- dir_cm2 is opposite of dir_cm

        double cos_theta_hadronic =
            dir_cm * trans / dir_cm.length() / trans.length();
        if (cos_theta_hadronic <
            0) { // <-- Here the dir_cm is set as the vector which has component
                 // || to the direction of boost
                 // <-- if cos(0)_cm is negetive then pi/2 <= theta <= pi, so
                 // this must have anti || component so reverse the // direction
                 // to have || component.
          dir_cm *= -1.0;
        ...
        if (pp) {
          out_nucleon_mec[0].set_momentum(
              dir_cm * sqrt(Ecm3 * Ecm3 -
                            Mass3 * Mass3)); // <- will always be leading proton
          out_nucleon_mec[1].set_momentum(
              dir_cm2 *
              sqrt(Ecm4 * Ecm4 -
                   Mass4 * Mass4)); // <- will always be subleading proton
        } else if (np) {
          out_nucleon_mec[0].set_momentum(
              dir_cm *
              sqrt(Ecm3 * Ecm3 -
                   Mass3 *
                       Mass3)); // <- neutron will have higher momentum in np
          out_nucleon_mec[1].set_momentum(
              dir_cm2 *
              sqrt(Ecm4 * Ecm4 -
                   Mass4 * Mass4)); // <- proton will have lower momentum in np
        } else if (pn) {
          out_nucleon_mec[0].set_momentum(
              dir_cm2 *
              sqrt(Ecm3 * Ecm3 -
                   Mass3 * Mass3)); // <- neutron will have lower momentum in pn
          out_nucleon_mec[1].set_momentum(
              dir_cm *
              sqrt(Ecm4 * Ecm4 -
                   Mass4 * Mass4)); // <- proton will have higher momentum in pn
        }

        out_nucleon_mec[0].boost2(-trans); // <<- Boost back to LAB frame
        out_nucleon_mec[1].boost2(-trans); // <<- Boost back to LAB frame

      } else { // <-- A piece of code if the mec_pauli_blocking is turned on in
               // data/params.txt

        int count_pb_trials =
            0; // <-- To keep count of pauli blocked trials.... dynamics should
               // be found in a finite number of trials.
        while (PB && count_pb_trials < ile_PB) {
          ...
          double kappa =
              (gamma * (np * Ecm4 + pn * Ecm3 + pp * Ecm4) - np * Mass4 -
               pn * Mass3 - pp * Mass4 - inc_nucleon_mec[0].his_fermi) /
              beta / gamma /
              (np * sqrt(Ecm4 * Ecm4 - Mass4 * Mass4) +
               pn * sqrt(Ecm3 * Ecm3 - Mass3 * Mass3) +
               pp * sqrt(Ecm4 * Ecm4 -
                         Mass4 * Mass4)); // <-- Allowed cosine boundary
                                          // determined by the nucleus which
                                          // have anti || component to boost

          if (!(gamma * (pp * Ecm4 + np * Ecm4 + pn * Ecm3) <=
                (pp * Mass4 + np * Mass4 + pn * Mass3 +
                 inc_nucleon_mec[0]
                     .his_fermi))) { // <- Pauli Blocking condiiton is checked
            PB = false; // <-- Set the PB flag false to get out of the do{ ...
                        // }while(PB && (licz < calls_max)); loop
            // dir_cm is sampled differently for different outgoing pairs /
            // sampling[0] corresponds for sampling direction for pp
            // sampling[1] corresponds for sampling direction for np
            // sampling[2] corresponds for sampling direction for pn;
            dir_cm = rand_direc2(
                trans, pp * sampling[0] + np * sampling[1] + pn * sampling[2],
                pp * strength[0] + np * strength[1] + pn * strength[2],
                TMath::Min(kappa, 1.0));
            dir_cm2 = -dir_cm; // <-- dir_cm2 is opposite of dir_cm
            ...
            double cos_theta_hadronic =
                dir_cm * trans / dir_cm.length() / trans.length();
            if (cos_theta_hadronic <
                0) { // <-- Here the dir_cm is set as the vector which has
                     // component || to the direction of boost
                     // <-- if cos(0)_cm is negetive then pi/2 <= theta <= pi,
                     // so this must have anti || component so reverse the //
                     // direction to have || component.
              dir_cm *= -1.0;
              dir_cm2 *= -1.0;
              cos_theta_hadronic =
                  dir_cm * trans / dir_cm.length() / trans.length();
            ...
            if (pp) {
              ...
              out_nucleon_mec[1].set_momentum(
                  dir_cm2 *
                  sqrt(Ecm4 * Ecm4 -
                       Mass4 * Mass4)); // <- Will always be sub-leading proton
            } else if (np) {
              out_nucleon_mec[0].set_momentum(
                  dir_cm * sqrt(Ecm3 * Ecm3 -
                                Mass3 * Mass3)); // <- Neutron will have higher
                                                 // momentum in np event.
              out_nucleon_mec[1].set_momentum(
                  dir_cm2 * sqrt(Ecm4 * Ecm4 -
                                 Mass4 * Mass4)); // <- Proton will have lower
                                                  // momentum in np event.
            } else if (pn) {
              out_nucleon_mec[0].set_momentum(
                  dir_cm2 * sqrt(Ecm3 * Ecm3 -
                                 Mass3 * Mass3)); // <- Neutron will have lower
                                                  // momentum in pn event.
              out_nucleon_mec[1].set_momentum(
                  dir_cm * sqrt(Ecm4 * Ecm4 -
                                Mass4 * Mass4)); // <- Proton will have higher
                                                 // momentum in pn event.
            }
...
void Generate_nucleon_kinematics_3p3h(particle *inc_nucleon_mec,
                                      particle *out_nucleon_mec, nucleus &T,
                                      double &xsec_result, vect qqq,
                                      int ile_PB) {
  ...
  double mass[3];
  double totalrestmass =
      0; // total rest mass of the system -> mass[0] + mass[1] + mass[2];
  ...
  do {
    ...
    licz++;
  } while (((suma.t < suma.length()) ||
            (suma * suma < totalrestmass * totalrestmass)) &&
           (licz < calls_max));

  if (((suma.t > suma.length()) &&
       (suma * suma >= totalrestmass * totalrestmass)) &&
      (licz < calls_max)) {
    ...
    if (PB) {
      while (PB && pb_trials < ile_PB) {
        threebodydecay(Winv, mass[0], mass[1], mass[2], nuc11, nuc22, nuc33);
        if ((nuc11.length() >= localfermi) && (nuc22.length() >= localfermi) &&
            (nuc33.length() >= localfermi)) {
          PB = false;
          break;
        }
        pb_trials++;
...
void threebodydecay(double W, double m1, double m2, double m3, vect &p1,
                    vect &p2, vect &p3) {
  if (W < m1 + m2 + m3) {
  ...
  do {
    double m12_2_los =
        (m1 + m2) * (m1 + m2) +
        frandom() * ((W - m3) * (W - m3) - (m1 + m2) * (m1 + m2));
    double m23_test = (m2 + m3) * (m2 + m3) +
                      frandom() * ((W - m1) * (W - m1) - (m2 + m3) * (m2 + m3));
    m23_2_los = m23_test;
  ...
  double kosalpha =
      (mom1 * mom1 + mom2 * mom2 - mom3 * mom3) / 2.0 / mom1 / mom2;
