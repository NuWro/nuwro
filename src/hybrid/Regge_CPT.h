...
void Regge( int process, int nucleon, int decay, int Helicity, double Qsq, double P[], double pN[], double Q[], double kpi[], Matrix QSlash, Matrix kpiSlash, double u, double s, double t, Matrix sSlash, Matrix uSlash, Matrix tSlash, Matrix PF_regge[], Matrix CT_regge[], Matrix NP_regge[], Matrix CNP_regge[] )
{  
...
// // // 
  ...
  if( icNP != 0 ){
    ...
    if( process == 1 || process == 2 ){
      AUX_np_A = GA * common;    
      for( int i=0; i<4; i++ ){
	NP_regge[i] = AUX_np_V*Gamma[i] - AUX_np_A*(Gamma_mu5[i] + (Q[i]/(Mpi2+Qsq))*QSlash_5); 
      }
    }
    
    if( process == 0 ){
      for( int i=0; i<4; i++ ){
	NP_regge[i] = AUX_np_V*Gamma[i] /*- (Fp*I*(icNP*sqrt(2.)/Qsq)*gPiNN)*(-Q[i]) * Gamma5*/; 
      }
    }
  ...
  if( icCNP != 0 ){
    ...
    if( process == 1 || process == 2 ){
      ...
      for( int i=0; i<4; i++ ){
	CNP_regge[i] = Gamma[i] * AUX_cnp_V - (Gamma_mu5[i] + (Q[i]/(Mpi2+Qsq))*QSlash_5)*AUX_cnp_A ; 
      }
    }
    if( process == 0 ){
      for( int i=0; i<4; i++ ){
	CNP_regge[i] = Gamma[i] * AUX_cnp_V /*+ (Fp_c*I*(icCNP*sqrt(2.)/Qsq)*gPiNN)*(-Q[i]) * Gamma5*/; 
      }
    }
  }
...
// 	AUX_PP = ( PRegge_rho * AUX_CT * Frho / (-Qsq-Mpi2) ) *QSlash;
  ...
  for( int i=0; i<4; i++ ){
	...
	if(nucleon == 1 && decay == 2){
	  CT_regge[i] = (AUX_CT * gA * Fp) * Gamma_mu5[i];
	}
	if(nucleon == 2 && decay == 2){
	  CT_regge[i] = (AUX_CT * gA * Fp_c) * Gamma_mu5[i];
	}
	...
	if(decay == 2){
	  CT_regge[i] = (AUX_CT * gA * wFCT) * Gamma_mu5[i] 
		      + AUX_CT_A* ( Gamma[i] + ( -krho/(4.*MN) )* (Gamma[i]*tSlash - tSlash*Gamma[i]) ) 
		      + Q[i]*AUX_PP;      
	}
    ...
    // 	  icNP = sqrt(1./2.); 
    // 	  icCNP = -sqrt(1./2.);
	      CT_regge[i] = (AUX_CT * gA * 0.5*(Fp+Fp_c)) * Gamma_mu5[i] 
			  + AUX_CT_A* ( Gamma[i] + ( -krho/(4.*MN) )* (Gamma[i]*tSlash - tSlash*Gamma[i]) ) 
			  + Q[i]*AUX_PP;
	    } 
	    if( decay==2 ){ // n(nu,mu-pi+)n 
