...
double sfevent(params &par, event &e, nucleus &t)
{
  ...
  if (method == 1)
  {
   ...
   else if (method == 2 || method == 3) // old SF implementations
   {
  ...
  const double focusingFactorSq = ( focusingFactorIncomingSq * focusingFactorOutgoingSq );
   ...
   if (doFSI)
   {
       const double Tk = tPPrime_approx( e.in[0].E(), l1.p().z / l1.momentum(), isElectron, !ccAllowed, m2, M ) ;
   ...
   pot_cascade = ( N1.pdg == pdg_neutron ) ? pot + factor * averCE : pot; // "De"correct again for cascade
   
   const double U_Correlation = std::max(0.0, pot_cascade);
   const double U_MeanField   = std::min(0.0, pot_cascade);
  ...
  const double minimalOmegaExact( ( coefASq * coefB - pow2(coefC) * l0.E() - coefC * root ) / ( pow2(coefB) - pow2(coefC) ) );
  if (l0eff.E() - l1.E() < minimalOmegaExact) return 0;
  
    // Approximated SRC (except argon SF)
    if ((method == 1 && !isAr40) || (method > 1))
    {
      if (doSRC && is_src(p, E, Z, N, !onNeutron) && (l0eff.t - l1.t - N1.Ek() - N2.Ek()) > srcOmegaCut)
        isCorrelated = true;
      else
        isCorrelated = false;
    }

  // Store event weight, flags, values and final-state particles
  ...
  if (isCorrelated && N2.momentum() > tinyP) e.out.push_back(N2); // Handle spectator nucleon

  e.averageCE = averCE;
  e.flag.isTransparent = isTransparent;
  e.optical_potential  = U_MeanField; 
