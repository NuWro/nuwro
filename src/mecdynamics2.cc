...
double kf_eff=220*pow(0.75, 1.0/3.0);//a parameter used in the oset_new function; for simplicity I took kf=225 as fixed
...
static double piondataset[9][6]={{5,  0,  0,  0,  0.79,  0.72},
...
{
	double wynik=omega+ 0.5*(omega*omega-q*q)/mecM;// /sqrt(M*M+0.6*kf*kf)
	;
	if (wynik<0) wynik=0;
	return wynik;
}
...
vec kier2 = vecprod (axis, kier1); // unit vetor -- new X axis
...
static void spowalniacz (double pot, vect &pp)//subtract energy by pot and adjusts momentum
{
...
return 2*(LinRelRe1(w,q)+LinRelRe2(w,q));}//wysumowana po izospinie
...
NR(w,q)-V_NN_t(w,q,RPA)*( NR(w,q)*NR(w,q)+NI(w,q)*NI(w,q) ))/den_t(w,q,RPA)
+ ND_old(w,q)*(-Pi/mecVol) / sqrt( fperf//*sqrt(mecM2+q*q)/mecM 
...
{      
  ...
  vec qq(cos(phi)*muonmom*sqrt(1.0 - cosmuon*cosmuon), sin(phi)*muonmom*sqrt(1.0 - cosmuon*cosmuon) , (E - muonmom*cosmuon));//momentum transfer
  
  vec kprim(-cos(phi)*muonmom*sqrt(1.0 - cosmuon*cosmuon), -sin(phi)*muonmom*sqrt(1.0 - cosmuon*cosmuon), muonmom*cosmuon);//muon momentum
  ...
  N22=vect(N2, length2-mecEf+mecM);//Fermi energy is subtracted
  ...
  while ( ( sumadd.t < sumadd.length() )   ||   ( sumadd*sumadd <(mass1+mass2)*(mass1+mass2) ) );//to be able to make Lorentz boost and to "decay"
    //cout<<"po"<<endl;
  trans= sumadd.v();
  ...
  vect nuc11=((Winv*Winv+mass1*mass1-mass2*mass2)/2.0/Winv,F1);  //hadron four momenta
...
{      
  ...
  while ( ( sumadd.t < sumadd.length() )   ||   ( sumadd*sumadd < (mass1+mass2+mass3)*(mass1+mass2+mass3)) );//to be able to make Lorentz boost and to "decay"
    //cout<<"po"<<endl;
  trans= sumadd.v();
  sumadd.boost2(trans);  //boost to the CM frame
