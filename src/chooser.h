...
public:
  ...
  Dyn &chooseDyn() {
    return proc[choose()];
  }                          ///< choose bin (according to bin weight)
  void set_weights_to_avg(); ///< make weights proportional to cross sections
  bool
  accept(int i, double x,
         double bias = 1); ///< add x and return true with probability xw/Mxw
  ...
  }              /// efficincy of i-th channel
  void report(); ///< report active channels characteristics
  void short_report(ostream &f,
                    bool format); ///< write calculated total cross sections for
                                  ///< each channel to file
  void calculate_counts(
      int ilosc); ///< calculate how many events to generate for each channel
  int desired(int i) {
    return proc[i].Desired;
  } ///< how many events to generate for each channel
  int ready(int i) {
    return proc[i].Ready;
  }; ///< how many events got accepted (and writen to file)
};
...
inline void chooser::reset(params &p) {
  proc.clear();
  if (p.dyn_qel_cc)
    proc.push_back(Dyn(0, "QELcc"));
  if (p.dyn_qel_nc)
    proc.push_back(Dyn(1, "QELnc"));
  if (p.dyn_res_cc)
    proc.push_back(Dyn(2, "REScc"));
  if (p.dyn_res_nc)
    proc.push_back(Dyn(3, "RESnc"));
  if (p.dyn_dis_cc)
    proc.push_back(Dyn(4, "DIScc"));
  if (p.dyn_dis_nc)
    proc.push_back(Dyn(5, "DISnc"));
  if (p.dyn_coh_cc)
    proc.push_back(Dyn(6, "COHcc"));
  if (p.dyn_coh_nc)
    proc.push_back(Dyn(7, "COHnc"));
  if (p.dyn_mec_cc)
    proc.push_back(Dyn(8, "MECcc"));
  if (p.dyn_mec_nc)
    proc.push_back(Dyn(9, "MECnc"));
  if (p.dyn_hyp_cc)
    proc.push_back(Dyn(10, "HYPcc"));
  if (p.dyn_lep)
    proc.push_back(Dyn(12, "LEP"));
  if (p.dyn_qel_el)
    proc.push_back(Dyn(20, "QELel"));
  if (p.dyn_res_el)
    proc.push_back(Dyn(21, "RESel"));
  if (p.dyn_mec_el)
    proc.push_back(Dyn(22, "MECel"));
  N = proc.size();
  do_distrib();
  if (proc[N - 1].Wacc == 0) {
...
////////////////////////////////////////////////////////////////////////
inline void chooser::add(int i, double x, double bias) { proc[i].add(x, bias); }
...
inline void chooser::short_report(ostream &f, bool format = false) {
  ...
  for (int k = 0; k < N; k++) {
    if (format)
      f << tab << line << endl << tab << " ";
    f << setw(3) << dyn(k) << " " << setw(10) << desired(k) << " " << setw(12)
      << ratio(k) << " " << setw(12) << avg(k) << endl;
  }
}
...
inline void chooser::report() {
  ...
  for (int j = 0; j < N; j++)
    cout << " |   " << setw(2) << proc[j].dyn << "  |   " << setprecision(6)
         << left << setw(5) << proc[j].label << right << "   | " << setw(12)
         << weight(j) << "     | " << setw(12) << ratio(j) << "     | "
         << setw(12) << efficiency(j) * 100 << " %   | " << endl
         << " |" << linia3 << "| " << setw(12) << avg(j) << " cm2 | "
         << setw(12) << sqrt(var(j)) << " cm2 | " << setw(12) << sigma(j)
         << " cm2 |" << endl;
