...
void NuWro ::refresh_dyn(params &par) { _procesy.reset(par); }
...
void NuWro::init(int argc, char **argv) {
  ...
  if (p.kaskada_redo == 0) {
    if (p.dyn_qel_el or p.dyn_mec_el or p.dyn_res_el) {
      cout << "     -> Loading e(p,p') fits..." << endl;
      BostedFit = new F1F209Wrapper();
      cout << "     -> Computing quasielasticpeak..." << endl;
      ComputeQuasiElasticPeak(p, *_nucleus, *BostedFit);
    }
  }
...
void NuWro::makeevent(event *e, params &p) {
  ...
  if (dyn < 6 || (dyn >= 10 && dyn < 12) || dyn == 20 || dyn == 21 ||
      dyn == 22) {
    ...
    case 2:
      ...
      if (p.dyn_res_cc) // res cc
      {
        ...
        case 1:
          ...
          break;
        case 2:
          resevent_hybrid(p, *e, *_nucleus, true);
          break;
        default:
          resevent2(p, *e, *_nucleus, true);
          break;
    ...
    case 8:
      ...
      // available
      {
        ...
        case 5:
          ...
          break;
        case 6:
          mecevent_2020Valencia(p, *e, *_nucleus, true);
          break;
        default:
          mecevent_tem(p, *e, *_nucleus, true);
          break;
  ...
  {
    switch ((int)p.NeuralNetwork_guided) {
    case 0:
      /*
       *
       *                  * Weights are as it is *
       *                  * for MEC e.weighte = 0 *  NOTE: No mode for MEC right
       * now
       *
       */
      switch (dyn) {
      ...
      case 21:
        e->flag.res = true;
        e->flag.nc = true;
        switch (p.eWro_spp_kind) {
        case 1:
          e_spp_event(p, *e, *_nucleus, false);
          break;
        case 2:
          e_spp_event2(p, *e, *_nucleus, false);
          break;
        case 3:
        ...
        break;
      case 22:
        std::cout << "Electron MEC event here !...\n";
        break;
      }
      break;
    ...
    case 1:
      /*
       *
       *              ** Inclusive weights are reweighted by DNN **
       *
       */
      switch (dyn) {
      case 20:
      case 21:
      case 22:
        DNN_event(p, *e, *_nucleus, *BostedFit, false);
        break;
      }
      break;
    default:
...
void NuWro::finishevent(event *e, params &p) {
  ...
  if (e->flag.res && e->flag.cc) {
    if (p.res_kind == 2 && e->flag.res_delta) {
      // for consistency reasons, the simplest solution for now is to recreate
      // the nucleus
  ...
  if (!e->flag.coh && !e->flag.lep &&
      (e->par.nucleus_n + e->par.nucleus_p > 1)) {
    kaskada k(p, *e, &input);
...
void NuWro::pot_report(ostream &o, bool format = false) {
  ...
  if (_detector) {
    ...
    o << tab << "Average BOX density  = " << _detector->density() / g * cm3
      << " g/cm3" << endl;
    o << tab << "Estimated BOX mass   = " << _detector->vol_mass() / kg << " kg"
      << endl;
...
void NuWro::test_events(params &p) {
  ...
  if (p.number_of_test_events > 0 && p.beam_test_only == 0) {
    ...
    if (p.save_test_events) {
      dismode = false;
      te = new TFile(
          (a.outputFileInfo._path + "weighted." + a.outputFileInfo._filename)
              .c_str(),
          "recreate");
    ...
    for (int i = 0; i < p.number_of_test_events; i++) {
      ...
      delete e;
      raport(i + 1, p.number_of_test_events, "test events ready... ", 1000, -1,
             "", bool(a.progress));
    } // end of nuwro loop
    if (p.save_test_events) {
      te->Write();
...
void NuWro::user_events(params &p) {
  ...
  for (A->start(); !A->end(); A->step()) {
    ...
    for (int i = 0; i < p.number_of_test_events; i++) {
      ...
      raport(i + 1, p.number_of_test_events, "analyser events ready... ", 1000,
             -1, "", bool(a.progress));
      p = p1;
    } // end of nuwro loop
...
void NuWro::real_events(params &p) {
  ...
  delete e;
  TH1 *xsections =
      new TH1D("xsections", "xsections", _procesy.size(), 0, _procesy.size());
  for (int i = 0; i < _procesy.size(); i++) {
    ...
    for (int k = 0; k < _procesy.size(); k++)
      if (_procesy.desired(k) > 0) {
        ...
        while (_procesy.ready(k) < _procesy.desired(k)) {
          ...
          raport(_procesy.ready(k), _procesy.desired(k), "events ready... ",
                 1000, _procesy.dyn(k), _procesy.label(k), bool(a.progress));
        }
        f1->Write();

        cout << "        100. % of " << _procesy.label(k) << " events ready..."
             << endl;
        ...
        if (p.mixed_order == 0) {
          ...
          for (int jj = start; jj < nn; jj++) {
            ...
            tf->Fill();
            delete e;
            raport(jj - start + 1, nn - start, "events copied... ", 100,
                   _procesy.dyn(k), _procesy.label(k), bool(a.progress));
          }
        }
  ...
  if (p.mixed_order) {
    ...
    while (ile > 0) {
      ...
      ile--;
      u[i]--;
      raport(nn - ile, nn, "real events copied... ", 100, -1, "",
             bool(a.progress));
    }
    delete e;
    for (int k = 0; k < _procesy.size(); k++)
  ...
  cout << "     " << "-> Generated the output file: \"" << output << "\""
       << endl;
  frame_bottom();
}
...
void NuWro::kaskada_redo(string input, string output) {
  ...
  for (int i = 0; i < nn; i++) {
    ...
    raport(i + 1, nn, " % events processed...", 100, e->dyn,
           _procesy.label(e->dyn), bool(a.progress));
  }
  cout << endl;
  fi->Close();
  delete fi;
